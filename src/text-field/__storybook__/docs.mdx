...
`TextField` is built with composition in mind — meaning that you need to combine
...
apply the red `color` to the field.
...
`Group` component. In this scenario, the `Input` can be passed `unstyled` to
...
TextFieldRootTextFieldRootTextField: TextFieldRootTextFieldRootTextFieldRoot
