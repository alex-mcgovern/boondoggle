...
const renderComponent = async (props: SelectMultiProps) => {
  /**
  ...
  return waitFor(async () => {
    return render(<SelectMulti {...props} />);
  });
};
...
describe("<SelectMulti />", () => {
  ...
  describe("Initial selected items", () => {
    it("should display '1 selected' as placeholder", async () => {
      const { getByRole } = await renderComponent({
        ...PROPS,
        initialSelectedItems: [FIRST_ITEM],
      });
  ...
  describe("Selecting with mouse", () => {
    ...
    it("should call `onChange` with second clicked item", async () => {
      ...
      expect(ON_CHANGE).toHaveBeenCalledWith(
        expect.objectContaining({
          selectedItems: [FIRST_ITEM, SECOND_ITEM],
        })
      );
    });

    it("should have placeholder with count of 1 clicked items", async () => {
      const { getByRole, getByText } = await renderComponent(PROPS);
      const combobox = getByRole("combobox");
      await userEvent.click(combobox);
  ...
  describe("Selecting with keyboard", () => {
    ...
    it("should call `onChange` with both items selected", async () => {
      renderComponent(PROPS);
